...
class EthRPC {
  ...
  async sendToAddress({ address, amount, passphrase, nonce }) {
    ...
    const account = await this.getAccount();
    nonce = nonce || await this.web3.getTransactionCount(account);
    ...
    const txid = await this.web3.eth.personal.sendTransaction(
      sendParams,
      passphrase
    );
    return { txid, nonce };
  ...
  async unlockAndSendToAddressMany({ payToArray, passphrase }) {
    ...
    for (const [i, request] of payToArray.entries()) {
      ...
      try {
        let txResult = await this.sendToAddress({ address, amount, passphrase, nonce });
        const { txid } = txResult;
